 ...
 * @author jorge
 */
public class DecisorRespuesta implements Decisor {
    ...
    public void setSituacion(Situacion situacion) {
        ...
        int apuestaActual = situacion.getApuestaActual();
        ...
        Decision decisionApuesta = decidirSegunApuesta(apuestaActual);
        
        int cantidad = decisionJugada.getCantidad() + decisionOrden.getCantidad() + decisionAnimo.getCantidad() + decisionApuesta.getCantidad();
        if (cantidad > 0) {            
            if (cantidad > 3) {
                accion = Accion.ENVIDAR;
                apuesta += random.nextInt(2 * cantidad);
            } else {
    ...
    private Decision decidirSegunApuesta(int n) {
        Decision decision;
        if (n >= 8) {        
            decision = Decision.SEGURAMENTE_NO;
        } else if (n >= 4) {
